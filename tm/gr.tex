...
\begin{align*}
	...
	D_r T_{mn} =& \partial_r T_{mn} - \Gamma\indices{^t_{rm}} T_{tn} - \Gamma\indices{^t_{rn}} T_{tm}
\end{align*}
...
\begin{align*}
	D_s d_r V_m =& D_s \big[ \partial_r V_m -\Gamma \indices{_{rn}^t} V_t\big]\\
	D_s d_r V_m - D_r d_s V_m =& R\indices{_{sr}^t_n} V_t \text{, where the Riemann curvature tensor is}\\
	R\indices{_{sr}^t_n} =& \partial_r \Gamma \indices{_{sn}^t} - \partial_s \Gamma \indices{_{rn}^t} + \Gamma \indices{_{sn}^p} \Gamma \indices{_{pr}^t}- \Gamma \indices{_{rn}^p} \Gamma \indices{_{ps}^t} 
\end{align*}

The curvature tensor indicates whether or not a space is flat, and it includes the today forces.
