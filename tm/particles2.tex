...
\begin{proof}
	...
	\begin{lemma}[Traceless Hermitian $2\times2$ matrix]\label{lemma:traceless:22}
		But any traceless Hermitian $2\times2$ matrix can be written as a linear combination of the Pauli  matrices.
	\end{lemma}
	\begin{proof}
		content...
	\end{proof}
...
\begin{table}[H]
	\begin{center}
		\caption{Electromagnetic and strong}
...
\begin{table}[H]
	\begin{center}
		\caption{Examples of Weak decay}
		\begin{tabular}{|l|l|p{6cm}|}\hline
			$n \rightarrow e + p +\bar{\nu}$&12 minutes&Barely enough energy: 940 $\rightarrow$ 939 + 0.5\\ \hline
			$\pi^- \rightarrow e^- + \bar{\nu} $&$10nS$&Plenty of energy\\ \hline
			$\pi^+ \rightarrow e^+ + \nu $&&\\ \hline
		\end{tabular}
	\end{center}
\end{table}
...
These are very slow: clearly something else is going on! Table \ref{tab:QCD:rows} is relevant. \begin{itemize}
	...
	\item $SU(2)$, a gauge symmetry, operates horizontally: $u\leftrightarrow d$;  $c\leftrightarrow s$;  $t\leftrightarrow b$.
\end{itemize}

\begin{table}[H]
	\begin{center}
		\caption{Quarks: Colour symmetry operates on first three rows, not columns}\label{tab:QCD:rows}
...
Let's see whether we can find a gauge boson that combines particle and anti, and boson,  apply it to leptons, e.g, $e \bar{\nu_e}$. We have a gauge boson, $W$:

 \begin{itemize}
 	\item $e \bar{\nu_e}\rightarrow W^- \rightarrow d\bar{u}$
 	\item $e^+ \nu_e\rightarrow W^+ \rightarrow \bar{d}u$
 \end{itemize}

Imagine that W are like photons or gluons.

\begin{figure}[H]
	\caption{W are like photons or gluons}
	\begin{subfigure}{0.45\textwidth}
...
\begin{figure}[H]
	...
	\end{subfigure}
	\begin{subfigure}{0.30\textwidth}
		\caption{Neutron decay. W doesn't have enough energy to muon, unless we hit neutron really hard.}
...
\url{https://youtu.be/3g6RtyGjX3o?t=3379}
