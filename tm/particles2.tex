...
Goal is to show how a particle like photon can gain a mass from spontaneous symmetry breaking; how it can gain amass from the Higgs mechanism. We will cover:
...
\subsection{Spontaneous symmetry breaking}
There is no fundamental difference between up and down. Answer can be determined by one spin very far away.
...
\begin{align*}
\mathcal{L} =& \frac{1}{2} \partial_\mu \Phi \partial^\mu \Phi - V(Phi)\text{, where V has the form of Figure \ref{fig:2-7-V}} \numberthis \label{eq:spontaneous:symmetry:L}
\end{align*}
...
Imagine a complex field, Figure \ref{fig:2-7-complex-phi}.
\begin{align*}
	\Phi =& \Phi_R + i \Phi_I\\
	\Phi^* =& \Phi_R - i \Phi_I\\
...
This is symmetric under: $\Phi \rightarrow \Phi^\prime=e^{i \theta} \Phi$
\begin{figure}[H]
	\caption{2-7-complex-phi}\label{fig:2-7-complex-phi}
...
\begin{appendices}
	...
	The  Brout-Englert-Higgs mechanism was postulated by Robert Brout, Fran\c{c}ois Englert and Peter Higgs. There are many moving parts, and it cannot be understood without quantum mechanics.
	...
	Figure \ref{fig:2-appendix-field-circle} depicts a circular orbit in "Field Space." It looks like angular momentum, so should be quantized. In the modern view,  a charged particle as an excitation of the field: it is made to spin around in field space.
	...
	Figure \ref{fig:mexican-hat} shows a different potential, with an unstable equilibrium at the origin. This is like a world where the vaccum has a non-zero field. The minimum is a circle centred on the origin: we can set a particle vibrating without expending any energy. If rotation corresponds to charge, we can get a charge without expending energy--a condensate.
	\begin{figure}[H]
	...
	There are two kinds of electrons, right handed and left handed. They can flip, but not if they were travelling at the speed of light (time infinitely slow); in the Dirac theory, mass is the rate at which it flips. See Figure \ref{fig:2-a3-flipping-electron}.
	...
	\caption{Electron flipping between left and right}
		...
		\end{subfigure}
		\begin{subfigure}[t]{0.3\textwidth}
			\caption{Electron emitting Z-boson. The coupling isn't charge--it is called "weak hypercharge". Prof Susskind calls it "zilch"}\label{fig:2-a3-Z-boson}
	...
	Figure \ref{fig:2-a3-zilch-Z} shows a Z-boson exchanging zilch with the condensate, so it oscillates between Z and Ziggs. It get mass. This is the Brout-Englert-Higgs phenomenon. The Ziggs was discovered in 1980: it is a part of the Z-boson.
	...
	The standard model is essentially correct. Is everything fitting together quantitatively? Figure \ref{fig:2-a3-higgs-neat} provides a hint. We should be able to run this backwards and decay Higgs, but it isn't easy to see gluons. Replace gluons with photons. Higgs can decay into photons. At the time of the lecture, Higgs appears to decay into photons a little too quickly.
	
	\url{https://youtu.be/JqNg819PiZY?t=3100} 
\end{appendices}
