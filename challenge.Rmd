...
```{r setrep,echo=FALSE}
n.repetitions<-get.n.repetitions(tau.data)
```

I used R to analyze the data, and a RMarkdown script to present the results, following the excellent advice of Rosanna van Hespen[@vanhespen2016thesis]. The model was executed by Behaviourspace, which performed `r n.repetitions` repetitions, varying the parameters listed in Table \ref{table:configs}

```{r echo=FALSE,results='asis'}
library(knitr)
kable(get.configurations(tau.data),caption="\\label{table:configs}Paramters varied by Behaviourspace")
...
Thanks are due to the SFI staff and mentors, and to my fellow students for discussions and recommended reading.
