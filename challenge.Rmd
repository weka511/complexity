...
I developed a Netlogo[@Wilensky:1999] model,  _challenge.nlogo_, which uses the approach of Fogel [@fogel1999inductive]---an evolutionary algorithm that breeds autoregressive models. It represents Investors and Pools as separate Breeds of Agents. Investors are connected to Pools by Links as shown in Figure \ref{fig:ui}. There are three types of Investors:
 ...
 1. the sharks in Figure \ref{fig:ui-cartel}  unlawfully form a cartel--Section \ref{heading:violate-sssumptions}. They share information and try to manipulate the numbers in one pool to maximise their profit.
...
Figures \ref{fig:plot_individual01} and \ref{fig:plot_individual25} show the growth of wealth for a few of the richest individuals (measured at the end of the run), a few of the poorest, and a few in the middle.  Notice that the rich are generally agents who showed their promise early, and kept ahead. Since it seems reasonable to expect a strategy to do better than random assignment, assuming it is any good, Figure \ref{fig:plot_individual_random} show the results of random decisions for comparison. Notice that rich individuals tend to be lucky several times, rather than the steady growth shown in Figures \ref{fig:plot_individual01} and \ref{fig:plot_individual25}.
...
##Violate Assumptions: allow  agents to observe the performance and of other  agents\label{heading:violate-sssumptions}
...
I wondered whether 100 steps were enough for the system to settle down, and for the intial conditions to be forgotten. I have used the concept of _ergodicity_ as a guide: very loosely, a system is ergodic if a sufficiently long part of its trajectory can be taken to be represent the entire trajectory. I encoded this as the following _operational definition_: the system can be considered ergodic, and the starting transients are insignificant, if the moving average of $n$ values is within one standard deviation of the mean taken over the entire trajectory. Figure \ref{fig:plot_ergodic} shows the results of taking $n=11$ and running the simulation for 250 steps, for different values of $\tau$, and varying initial allocations to the low and high risk pools. Notice that the transient has disappeared for most runs for $\tau=0$, but most runs are chaotic for $\tau>0$. In retrospect I believe that 100 steps are adequate if the tranjectory is not chaotic; if it is chaotic, increasing the number of steps is unlikely to lead to help. 
...
```{r echo=FALSE,fig.cap="\\label{fig:plot_sq0}Variation of Error for $\\tau=0$. Colour is used to distinguish one run for another, and has no other significance.",warning=FALSE, out.width='.49\\linewidth', fig.width=4, fig.height=4,fig.show='hold',fig.align='center',fig.pos = 'p',out.extra = ''}
...
```{r echo=FALSE,fig.cap="\\label{fig:plot_sq1}Variation of Error for $\\tau=1$. Colour is used to distinguish one run for another, and has no other significance.",warning=FALSE, out.width='.49\\linewidth', fig.width=4, fig.height=4,fig.show='hold',fig.align='center',fig.pos = 'p',out.extra = ''}
...
```{r echo=FALSE,fig.cap="\\label{fig:plot_individual25}Growth of wealth for $\\tau \\in \\{2,5\\}$. The figures on the left use linear predictors, those on the right are conservative.",warning=FALSE, out.width='.49\\linewidth', fig.width=4, fig.height=4,fig.show='hold',fig.align='center',fig.pos = 'p',out.extra = ''}
...
```{r echo=FALSE,fig.cap="Sharks represent members of the Cartel. It appears that cartel members are able to intimidate agents who use the first learning algorithm, but not the second.\\label{fig:ui-cartel}",warning=FALSE, out.width='.49\\linewidth', fig.width=4, fig.height=4,fig.show='hold',fig.align='center',fig.pos = 'p',out.extra = ''}
knitr::include_graphics('Cartel.jpg')
```


```{r plot_cartel, echo=FALSE,fig.cap="\\label{fig:cartel_return}Return from Cartel, compared with Stable Pool",warning=FALSE, out.width='.49\\linewidth', fig.width=4, fig.height=4,fig.show='hold',fig.align='center',fig.pos = 'p',out.extra = ''}
cartel.data<-read.cartel()
plot.cartel(cartel.data )
```


```{r plot_wealth, echo=FALSE,fig.cap="\\label{fig:tau_wealth}Variation of accumulated wealth with $\\tau$",warning=FALSE, out.width='.49\\linewidth', fig.width=4, fig.height=4,fig.show='hold',fig.align='center',fig.pos = 'p',out.extra = ''}
endrun.data<-get.tau(file.name='take2 experiment-table-new.csv')
...
```{r plot_errors, echo=FALSE,fig.cap="\\label{fig:tau_error}Variation of Error with $\\tau$",warning=FALSE, out.width='.49\\linewidth', fig.width=4, fig.height=4,fig.show='hold',fig.align='center',fig.pos = 'p',out.extra = ''}
...
```{r plot_many_investors, echo=FALSE,fig.cap="\\label{fig:many_investors}Many Investors, showing number in each pool. The reference line shows the maximum number of subscribers if pool is to be profitable.",warning=FALSE, out.width='.49\\linewidth', fig.width=4, fig.height=4,fig.show='hold',fig.align='center',fig.pos = 'p',out.extra = ''}
...
my.individual.details<-read.csv('C:/Users/Weka/201804/Experiments/envy1.csv')
plot.individuals(my.individual.details,my.strategy=1,n=5)
plot.individuals(my.individual.details,my.strategy=0,n=5)
my.individual.details<-read.csv('C:/Users/Weka/201804/Experiments/envy1_5.csv')
...
```{r echo=FALSE,fig.cap="\\label{fig:plot_individual_envy23}The effects of allowing agents to spy on each other (continued).",warning=FALSE, out.width='.49\\linewidth', fig.width=4, fig.height=4,fig.show='hold',fig.align='center',fig.pos = 'p',out.extra = ''}
my.individual.details<-read.csv('C:/Users/Weka/201804/Experiments/envy2.csv')
...
my.individual.details<-read.csv('C:/Users/Weka/201804/Experiments/envy3.csv')
