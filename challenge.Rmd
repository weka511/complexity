 ...
 * The first followed the approach of [@arthur1994inductive], using a "bag of strategies" to assign agents to pools. The strategies include _stay in pool_, _jump at random_, and _estimate length_--- to select pool offering the best likely return. Each agent kept track of the performance of the strategies in its bag, and replaced them if they are not doing well. The limitation of this  approach is that new strategies cannot arise.
  * _Challenge.nlogo_ uses the approach of Fogel [@fogel1999inductive]---a genetic algorithm that breeds autoregressive models. It represents Investors and Pools as separate Breeds of Agents. Investors are connected to Pools by Links as shown in Figure \ref{fig:ui}. There are three types of agents:
    ...
    1. the wolves unlawfully form a cartel. They share information and try to manipulate the numbers in one pool to maximise thier profit. They are discussed in Section \ref{heading:violate-sssumptions}. 
...
_Suppose that meta-agents exist that can coordinate the behaviors of a subset of the agents (and split the resulting payoffs equally across the subset)---how does this impact the system's behavior?_

I implemeneted a _cartel_, where a group of agents attemp to monopolize one pool for their own benefit. Figure \ref{fig:ui-cartel} uses wolves to depict members of the cartel.
...
###   Varying of total number of agents
...
```{r echo=FALSE,fig.cap="Wolves represent members of the Cartel. It appears that cartel members are able to intimidate agents who use the first learning algorithm, but not the second.\\label{fig:ui-cartel}",warning=FALSE, out.width='.49\\linewidth', fig.width=4, fig.height=4,fig.show='hold',fig.align='center',fig.pos = 'p'}
